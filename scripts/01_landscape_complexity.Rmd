...
Firt, we need to define a function to call orfeo directly from R. This is going to call the meanshift segmentation tool.
...
```{r segvis}
segs = dir("../results/simulated_scapes/seg",pattern=".shp$",full.names=T)
cs = readOGR(segs[1])
ns = readOGR(segs[2])
...
rasterOptions(tmpdir="D:/Arthur/temp")
...
bands4i = append(bands4,c("NDVI","WAVI"))
bands8i = append(bands8,c("NDVI","WAVI"))

if(length(colnames(extract))>8){
colnames(extract) = bands8i
}

if(length(colnames(extract))<8){
colnames(extract) = bands4i
}

# hist(extract$N);hist(extract$NDVI);hist(extract$WAVI)

psp@data = cbind(psp@data,extract)
psp2 = sp.na.omit(psp)
psp2 = psp2[psp2@data$R>0,]

water_mask = psp2[psp2@data$WAVI<0.25,]
# water_mask = psp2[psp2@data$WAVI<0.25,]
# plot()
# plot(water_mask)
# plot(r$N);plot(r$WAVI)

water_mask2 = water_mask[water_mask@data$N<0.35,]


wmsf = st_as_sf(water_mask2)
wmsfc = st_union(wmsf)

skip_to_next <- FALSE

water = tryCatch(
  mask(r, as_Spatial(wmsfc)), error = function(e) {skip_to_next <<- TRUE}
)

if(skip_to_next){next}

writeRaster(water, paste0("//Glaciolab/homes/degranda/MFFP/digitalglobe_archives/water_mask/",str_sub(imgs[i],-46)), overwrite=T)
 }
```
